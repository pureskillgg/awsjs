...
test('putObjectJson: throws error from client', async (t) => {
  const { AwsS3Client, createClient } = t.context
  const client = createClient(t)
...
test('getObjectJson: returns gzipped response', async (t) => {
  const { AwsS3Client, createClient } = t.context
  const client = createClient(t)
  const key = 'mock-key'
  const contentEncoding = 'gzip, none'
  ...
  td.when(
    ...
    ...getObjectResponse(),
    Body: toStream(gzipSync(Buffer.from('{"foo":2}'))),
  ...
  t.deepEqual(data, {
    ...{ ...getObjectResponseFormatted, contentEncoding },
    data: { foo: 2 }
  })
})
...
test('getObjectJson: throws error from client', async (t) => {
  const { AwsS3Client, createClient } = t.context
  const client = createClient(t)
  const err = new Error('foo')
...
const getObjectResponse = () => ({
  // UPSTREAM: https://github.com/aws/aws-sdk-js-v3/issues/1877
  Body: toStream('{"foo":2}'),
  DeleteMarker: true,
