...
test.beforeEach((t) => {
  t.context.DocumentClient = td.constructor(['put', 'update'])

  t.context.createClient = (t, options) => {
    const client = new DynamodbDocumentClient({
      DocumentClient: t.context.DocumentClient,
      hashKey,
      rangeKey,
      reqId,
      log: createLogger({ t }),
...
test('constructor: passes tableName to AWS DocumentClient', (t) => {
  const { DocumentClient } = t.context
  ...
  td.verify(new DocumentClient({ params: { TableName: tableName } }))
  t.truthy(client)
})

test('constructor: passes params to AWS DocumentClient', (t) => {
  const { DocumentClient } = t.context
  const tableName = 'some-table-name'
  ...
  td.verify(new DocumentClient({ params: { ...params, TableName: tableName } }))
  t.truthy(client)
})
...
test('put: returns response', async (t) => {
  const { DocumentClient, createClient } = t.context
  const client = createClient(t)
  const promise = td.func()
  const input = { [hashKey]: 'foo', [rangeKey]: 'bar' }

  td.when(DocumentClient.prototype.put({ Item: input })).thenReturn({
    promise
  })
...
test('put: passes params', async (t) => {
  ...
  td.when(DocumentClient.prototype.put({ Item: input, Bar: 3 })).thenReturn({
    promise
  })
...
test('put: throws client error', async (t) => {
  ...
  td.when(DocumentClient.prototype.put(td.matchers.anything())).thenReturn({
    promise
  })
...
test('update: returns response', async (t) => {
  ...
  td.when(DocumentClient.prototype.update({ Key: key })).thenReturn({
    promise
  })
...
test('update: passes params', async (t) => {
  ...
  td.when(DocumentClient.prototype.update({ Key: key, Bar: 3 })).thenReturn({
    promise
  })
...
test('update: throws client error', async (t) => {
  ...
  td.when(DocumentClient.prototype.update(td.matchers.anything())).thenReturn({
    promise
  })
  td.when(promise()).thenReject(err)
